...
plot(sin,0:2π) # Grafica "sin" en el rango 0:2π #plot une los pares ordenados con lineas rectas 
...
scatter(sin,0:2π) # Tabula "sin" en el rango 0:2π #scatter: solo para graficar puntos (pares ordenados)
...
plot(sin,0:0.05:2π) # ¡Mucho mejor! :D # plot(función, inicio del intervalo: tamaño de paso:punto final del intervalo (rango))
...
begin
    l = 2         # ¡Cambia el valor de l y corre la celda, #l=tamaño de los intervalors  #al cambiar los valores de l cambian las gráficas se modifican 
...
@bind l1 Slider(0.05:0.05:2, default=0.5) #Slider(inicio del rango: intervalo enytre el que se puede mover l1: final del rango, default=valor predeterminado de l1)

# ╔═╡ cdd16f8e-e945-4d5f-a71d-6896f9146512
l1 #se crea la variable l1 cuyo valor se determina deslizando la barra

# ╔═╡ 576a42f0-bd11-4480-9e97-2f9098312a25
plot(cos,0:l1:2π) #primera gráfica plot=se unen los puntos 

# ╔═╡ 219a5b8d-af3a-4023-8904-902e7b4e45e7
mifunción(x)=x^2 #puedes tu mismo definir tu función 

# ╔═╡ 35d05477-a09e-4077-8513-2ac7dba4def1
scatter(cos,0:l1:2π) #segunda gráfoca scatter: se gráfican solamente los puntos 
...
@bind l2 Slider(0.05:0.05:2, default=0.5) #barra para deslizar 

# ╔═╡ d6a012c7-47b6-4374-8522-aa8e6efbe84f
l2 #definir variable l2

# ╔═╡ 4d68a04e-6502-4478-a9c2-8a81bcc2b282
begin #bloque de código inicia con begin termina con end 
...
begin #equivalente al código anterior, lo unico que cambia son los colores (y las leyendas) por default, primero azul y luego naranja 
...
begin
	plot(sin,0:0.25:2π, title = "plot y scatter", xlabel = "x", ylabel = "sin(x)", color = "darkorange", label = "plot") #plot(función, inicio rango:tamaño de paso: fin del rango, titulo= "entre comillas",xlabel=, ylabel=,color=, label="leyenda, por ejemplo plor que sería lo primero que se gráfica" )
    scatter!(sin,0:0.25:2π, color = "green", label = "scatter", marker = :diamond) #marker=: figura
end
...
# ╔═╡ 3d32e083-c14d-4855-91a1-4b5c1bdb94e7
@bind l3 Slider(0.05:0.05:2, default=0.5) #creamos el parámetro d para controlar el nivel de detalle de la gráfica, ahora con una nueva variable l3

# ╔═╡ a60bca26-1ad9-4d12-945f-2654921f11fb
l3 #definimos una nueva variable l3 para utilizar Slider 

# ╔═╡ 4ff5ba06-4d68-4f55-ab51-23a163265e58
begin #iniciamos el código
	plot(sin,-π:l3:π, title = "Funciones tigonométricas", xlabel = "x", ylabel = "sin(x) y cos(x)", color = "green", label = "sin(x)") #gráficamos primero la función sen(X) y le asignamos el color verde y la etiqueta "sen(x)" usando los atributos de figuras 
	plot!(cos,-π:l3:π, color = "brown2", label = "cos(x)") #ahora utilizando pot! gráficamos cos(x) dentro de la misma gráfica (encima), le asignamos un color diferente y su etiqueta correspondiente a la función 
end #terminamos el bloque de código 
...
# ╔═╡ a710223e-7382-4767-8e5c-91a06c9ad506
md"""Recordemos que las ecuaciones cinemáticas para un tiro parabólico son:

y = -1/2 g t^2 + v(sen(θ))t + h #Esta corresponde a la ecu, cinemática de y 

donde g=9.81m/s^2

Vy= -gt + v(sen(θ)) #Ecu. cinémática de velocidad en y 

x= v(cos(θ))t + X0 #Ecu. de posición en x

donde X0= 0 #Posición incial """


# ╔═╡ 34e7985b-f9e8-49d7-9079-da1916f2a489
...
begin 
	...
	H(x,y)=cos(x)+sin(y) #definimos la función

@gif for Y in (0:0.01:2π) #Para cada valor del rango del eje Y que se encuentre entre 0 y 2π (dando pasos de 0.01)
	surface((0:0.05:2π),(0:0.05:π*Y),H, color = :devon) #Generamos una superficie, en donde el eje X va de o a 2π; mientras que en el eje va desplazandose a lo largo de la superficie 
    end
end
